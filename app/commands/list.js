...
module.exports = function (connection, parsed, data, callback) {
    ...
    if (!parsed.attributes || parsed.attributes.length !== 2 || !parsed.attributes[0] || ["STRING", "LITERAL", "ATOM"].indexOf(parsed.attributes[0].type) < 0 || !parsed.attributes[1] || ["STRING", "LITERAL", "ATOM"].indexOf(parsed.attributes[1].type) < 0) {
    ...
    if (!parsed.attributes[1].value) {
        ...
        folders.forEach(function (folder) {
            connection.send({
                tag: "*",
                command: "LIST",
                attributes: [folder.flags.map(function (flag) {
                    return {
                        type: "ATOM",
                        value: flag
                    };
                }), connection.server.storage[folder.namespace].separator, folder.path]
