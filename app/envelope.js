...
module.exports = function (header) {
    return [header.date || null, header.subject || "", processAddress(header.from), processAddress(header.sender, header.from), processAddress(header["reply-to"], header.from), processAddress(header.to), processAddress(header.cc), processAddress(header.bcc),
    // If this is an embedded MESSAGE/RFC822, then Gmail seems to
    // have a bug here, it states '"NIL"' as the value, not 'NIL'
    header["in-reply-to"] || null, header["message-id"] || null];
};
...
function processAddress(arr, def) {
    ...
    arr.forEach(function (addr) {
        if (!addr.group) {
            result.push([addr.name || null, null, // FIXME: check the rfc, this should be related to groups
