...
module.exports = function (server) {
    ...
    var createHandler = server.getCommandHandler("CREATE"),
        allowedList = [].concat(server.options["special-use"] || ["\\Archive", "\\Drafts", "\\Flagged", "\\Junk", "\\Sent", "\\Trash"]);
    ...
    server.setCommandHandler("CREATE", function (connection, parsed, data, callback) {
        ...
        if (parsed.attributes && Array.isArray(parsed.attributes[1])) {
            for (i = 0; i < parsed.attributes[1].length; i += 2) {
                if (parsed.attributes[1][i] && parsed.attributes[1][i].type === "ATOM" && parsed.attributes[1][i].value.toUpperCase() === "USE" && Array.isArray(parsed.attributes[1][i + 1])) {
