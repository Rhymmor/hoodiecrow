...
module.exports = function (server) {
    ...
    server.setCommandHandler("AUTHENTICATE XOAUTH2", function (connection, parsed, data, callback) {
        ...
        if (parsed.attributes.length !== 1 || !parsed.attributes[0] || ["STRING", "ATOM"].indexOf(parsed.attributes[0].type) < 0) {
            connection.send({
                tag: parsed.tag,
                command: "NO",
                attributes: [{
        ...
        if (parts.length !== 4 || !parts[0].match(/^user\=/) || !parts[1].match(/^auth\=Bearer /) || !user || // Must be present
        ...
        if (!connection.server.users.hasOwnProperty(user) || !connection.server.users[user].xoauth2 || connection.server.users[user].xoauth2.accessToken !== accessToken) {
