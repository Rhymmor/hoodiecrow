...
module.exports = function (server) {
    ...
    var moveHandler = function moveHandler(uidMode, connection, parsed, data, callback) {
        function uidify(str) {
            if (uidMode) {
                return "UID " + str;
            }
        ...
        if (!parsed.attributes || parsed.attributes.length !== 2 || !parsed.attributes[0] || ["ATOM", "SEQUENCE"].indexOf(parsed.attributes[0].type) < 0 || !parsed.attributes[1] || ["ATOM", "STRING"].indexOf(parsed.attributes[1].type) < 0) {
            connection.send({
                tag: parsed.tag,
                command: "BAD",
                attributes: [{
        ...
        var rangeMessages = range.map(function (x) {
            return x[1];
        });
