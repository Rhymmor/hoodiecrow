...
module.exports = function (server) {
    ...
    var selectHandler = server.getCommandHandler("SELECT"),
        ...
        condstoreHandler = function condstoreHandler(prevHandler, connection, parsed, data, callback) {
    ...
    server.setCommandHandler("FETCH", function (connection, parsed, data, callback) {
        ...
        if (changedsince) {
            if (["ATOM", "STRING"].indexOf(changedsince.type) < 0 || !changedsince.value.length || isNaN(changedsince.value) || Number(changedsince.value) < 0) {
    ...
    server.setCommandHandler("STORE", function (connection, parsed, data, callback) {
        ...
        if (unchangedsince) {
            if (["ATOM", "STRING"].indexOf(unchangedsince.type) < 0 || !unchangedsince.value.length || isNaN(unchangedsince.value) || Number(unchangedsince.value) < 0) {
